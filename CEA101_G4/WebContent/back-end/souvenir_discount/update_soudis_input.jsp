...
<%
	SouvenirDiscountVO sodVO = (SouvenirDiscountVO) request.getAttribute("sodVO"); //EmpServlet.java (Concroller) 存入req的empVO物件 (包括幫忙取出的empVO, 也包括輸入資料錯誤時的empVO物件)
%>
<%=sodVO == null%>--${sodVO.sou_dis_id}--
<html>
<head>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1" />
<title>特產促銷修改 - update_soudis_input.jsp</title>
...
<body bgcolor='white'>
	...
	<table id="table-1">
		<tr>
			<td>
				<h3>特產促銷資料修改 - update_soudis_input.jsp</h3>
				<h4>
					<a href="/CEA101G4/back-end/souvenir_discount/select_soudis_page.jsp">回首頁</a>
				</h4>
			</td>
		</tr>
	</table>
	...
	<FORM METHOD="post"
		...
		<table>
			<tr>
				<td>特產促銷編號:<font color=red><b>*</b></font></td>
				<td><%=sodVO.getSou_dis_id()%></td>
			</tr>
<!-- 			<tr> -->
<!-- 				<td>特產類別:</td> -->
...
<!-- 			</tr> -->
	...
	<tr>
		...
		<td><select size="1" name=sou_type_id>
			...
			<tr>
				<td>特產促銷商品名稱:</td>
				<td><input type="TEXT" name="sou_dis_name" size="45"
					value="<%=sodVO.getSou_dis_name()%>" /></td>
			</tr>
			<tr>
				<td>特產促銷開始日期:</td>
				<td><input type="TEXT" id="f_date1" name="sou_dis_start" size="45"
					value="<%=sodVO.getSou_dis_start()%>" /></td>
			</tr>
			<tr>
				<td>特產促銷結束日期:</td>
				<td><input type="TEXT" id="f_date2" name="sou_dis_end" size="45"
					value="<%=sodVO.getSou_dis_end()%>" /></td>
			</tr>
			<tr>
				<td>特產促銷折扣:</td>
				<td><input type="TEXT" name="sou_dis_status" size="45"
					value="<%=sodVO.getSou_dis_status()%>" /></td>
			</tr>


		</table>
		<br> <input type="hidden" name="action" value="update"> <input
			type="hidden" name="sou_dis_id" value="<%=sodVO.getSou_dis_id()%>"> <input
...
<%
	...
	try {
		sou_dis_start = sodVO.getSou_dis_start();
	} catch (Exception e) {
...
<%
	...
	try {
		sou_dis_end = sodVO.getSou_dis_end();
	} catch (Exception e) {
...
<script>
        ...
        $('#f_date1').datetimepicker({
		   ...
		   value: '<%=sou_dis_start%>', // value:   new Date(),
        ...
        $('#f_date2').datetimepicker({
	       ...
	       step: 5,                //step: 60 (這是timepicker的預設間隔60分鐘)
	       format:'Y-m-d H:i:s',         //format:'Y-m-d H:i:s',
		   value: '<%=sou_dis_end%>', // value:   new Date(),
	//disabledDates:        ['2017/06/08','2017/06/09','2017/06/10'], // 去除特定不含
	//startDate:	            '2017/07/10',  // 起始日
	//minDate:               '-1970-01-01', // 去除今日(不含)之前
	//maxDate:               '+1970-01-01'  // 去除今日(不含)之後
