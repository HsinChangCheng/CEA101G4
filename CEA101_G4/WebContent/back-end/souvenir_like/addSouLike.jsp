...
<%
	SouvenirProductLikeRecordVO souprVO = (SouvenirProductLikeRecordVO) request.getAttribute("souprVO");
%>
...
<title>特產按讚新增 - addSouLike.jsp</title>
...
<!-- 							<p> -->
<!-- 								<label for="namelabel">特產id:</label><input type="text" -->
<!-- 									name="sou_id" size="45" -->
<%-- 									value="<%=(souprVO == null) ? "SOU001" : souprVO.getSou_id()%>"> --%>
<!-- 							</p> -->
									...
									<label for="typelabel">特產編號:</label> <SELECT size="1"
										...
										<c:forEach var="soupVO" items="${soupSvc.all}">
											<option value="${soupVO.sou_id}"
												${(souprVO.sou_id==soupVO.sou_id)? 'selected':'' }>${soupVO.sou_id}
										</c:forEach>
									</SELECT>
							...
							<p>
								<label for="namelabel">會員id:</label><input type="text"
									name="mem_id" size="45"
									value="<%=(souprVO == null) ? "MEM002" : souprVO.getMem_id()%>">
							</p>
							<p>
							<div id="preview"></div>
						</div>
